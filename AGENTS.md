...
The `@/lib/matthews-graphql/server` barrel (`auth`, `query`, `PreloadQuery`, `safeQuery`, `requireSession`, `SafePreload`) uses Node-only code (the RSC Apollo client and the NextAuth instance). Importing it from a route or component that sets `export const runtime = "edge"` throws at module load — on purpose. **The only edge-safe export is `@/lib/matthews-graphql/proxy`**, wired through `src/proxy.ts`. If you need session data on the edge, use the proxy — don't import `/server`. See `src/proxy.ts` for the canonical example.
...
- For Server Components, import `query`, `PreloadQuery`, `safeQuery`, `SafePreload`, `requireSession`, and `auth` from `@/lib/matthews-graphql/server`.
...
- **Server Actions** and **Route Handlers** (`route.ts` under `/api`) are server context — import `auth`, `query`, `PreloadQuery`, `safeQuery`, `SafePreload`, `requireSession` from `/server` exactly as you would in an RSC.
- **Edge runtime** is a separate world — see the callout at the top of this document. Don't import `/server` from an edge handler; use `@/lib/matthews-graphql/proxy`.
